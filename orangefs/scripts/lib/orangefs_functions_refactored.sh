#!/bin/bash
# orangefs_functions_refactored.sh - Stateless OrangeFS installation and deployment
  ...
  if ! timeout 300 parallel-ssh -P -h "${server_list_file}" -t 60 -O "StrictHostKeyChecking=no" -O "BatchMode=yes" \
    "log_dir='/tmp/orangefs/logs' && \
     ...
     { \
       ...
       source ${script_root}/lib/orangefs_functions_refactored.sh && configure_orangefs_firewall; \
     } > \"\${log_file}\" 2>&1"; then
