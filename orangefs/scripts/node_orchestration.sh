...
write_node_environment_file() {
...

  {
    ...
    echo ""
    echo "export NODE_ROLE=\"${node_role}\""
    echo "export TARGET_USER=\"${target_user}\""
    echo "export IS_LOGIN_NODE=\"${is_login_node}\""
    echo "export IS_COMPUTE_NODE=\"${is_compute_node}\""
    echo "export IS_STORAGE_NODE=\"${is_storage_node}\""
    echo ""
    echo "# Logging"
    echo "export LOG_LEVEL=\"${log_level}\""
    echo ""
    echo "# SSH Configuration"
    echo "export PUBLIC_KEY_SET=\"$([ -n \"${public_key}\" ] && echo 'yes' || echo 'no')\""
    echo "export PRIVATE_KEY_SET=\"$([ -n \"${private_key}\" ] && echo 'yes' || echo 'no')\""
    echo ""
    echo "# NFS Configuration"
    echo "export NFS_SERVER_IP=\"${nfs_server_ip}\""
    echo "export NFS_MOUNT_POINT=\"${nfs_mount_point}\""
    echo "export NFS_EXPORT_DIR=\"${nfs_export_dir}\""
    echo ""
    ...
    echo "export OS_APPLICATION_CREDENTIAL_ID_SET=\"$([ -n \"${os_app_cred_id}\" ] && echo 'yes' || echo 'no')\""
    echo "export OS_APPLICATION_CREDENTIAL_SECRET_SET=\"$([ -n \"${os_app_cred_secret}\" ] && echo 'yes' || echo 'no')\""
    echo ""
    echo "# Script Paths"
